...
def clean_flag_description(flag_sources):
    ...
    for source in sources:
        ...
        elif 'Iowa Lobbyist' in source:
            # Special handling for Iowa Lobbyist - just show "Iowa Lobbyist"
            clean_descriptions.append("Iowa Lobbyist")
...
def generate_email_report(flagged_donors):
    """Generate email-ready report for high confidence matches, split by megadonors and other flags"""
    if flagged_donors.empty:
        return ""
    ...
    # Split into megadonors and other flags
    nrcc_donors, republican_donors, rga_donors, other_donors = filter_megadonors(high_priority)
    ...
    if len(nrcc_donors) + len(republican_donors) + len(rga_donors) > 0:
        report_lines.append("=== MEGADONORS ===\n")

        # NRCC Megadonors
        if len(nrcc_donors) > 0:
            report_lines.append("NRCC Megadonors:")
            for _, donor in nrcc_donors.iterrows():
                amount = donor.get('amount', '$0.00')
                amount_numeric = donor.get('amount_numeric', 0)
                line = f"• {donor['first_name']} {donor['last_name']}, {amount}"
                report_lines.append(line)
                megadonor_total += amount_numeric
                megadonor_count += 1
            report_lines.append("")

        # Republican Megadonors
        if len(republican_donors) > 0:
            report_lines.append("Republican Megadonors:")
            for _, donor in republican_donors.iterrows():
                amount = donor.get('amount', '$0.00')
                amount_numeric = donor.get('amount_numeric', 0)
                line = f"• {donor['first_name']} {donor['last_name']}, {amount}"
                report_lines.append(line)
        ...
        # RGA Donors
        if len(rga_donors) > 0:
            report_lines.append("RGA Donors:")
            for _, donor in rga_donors.iterrows():
                amount = donor.get('amount', '$0.00')
                amount_numeric = donor.get('amount_numeric', 0)
                line = f"• {donor['first_name']} {donor['last_name']}, {amount}"
                report_lines.append(line)
        ...
        report_lines.append(f"Megadonor Subtotal: ${megadonor_total:,.2f} ({megadonor_count} donors)")
        report_lines.append("")
    ...
    if len(other_donors) > 0:
        report_lines.append("=== OTHER HIGH PRIORITY FLAGS ===\n")

        for _, donor in other_donors.iterrows():
            first_name = donor['first_name']
            last_name = donor['last_name']
            amount = donor.get('amount', '$0.00')
            amount_numeric = donor.get('amount_numeric', 0)

            # Get flag description using the same function as the dashboard
            flag_sources = donor.get('flag_sources', '')
            flag_desc = clean_flag_description(flag_sources)

            line = f"• {first_name} {last_name}, {amount}, {flag_desc}"
            report_lines.append(line)

            other_total += amount_numeric
            other_count += 1

        report_lines.append(f"\nOther Flags Subtotal: ${other_total:,.2f} ({other_count} donors)")
        report_lines.append("")

    # === GRAND TOTAL ===
    grand_total = megadonor_total + other_total
    grand_count = megadonor_count + other_count

    report_lines.append("=== GRAND TOTAL ===")
    report_lines.append(f"Total: ${grand_total:,.2f} ({grand_count} donors)")
...
def generate_fec_financial_email_report():
    ...
    report_lines.append(f"Total Number of Itemized Contributions: {total_contributions:,}")
    report_lines.append(f"Median Itemized Contribution: ${median_contribution:.2f}")
    report_lines.append(f"Number of Max Out Contributions: {max_out_donors:,}")
    
    return "\n".join(report_lines)

def filter_megadonors(flagged_donors):
    """
    Filter donors into megadonor categories and remaining donors
    Returns: (nrcc_donors, republican_donors, rga_donors, remaining_donors)
    """
    if flagged_donors.empty:
    ...
    # Helper function to check if donor matches a megadonor type
    def is_megadonor_type(donor, megadonor_name):
        # Check flag_sources first (most reliable)
        flag_sources = donor.get('flag_sources', '')
        if megadonor_name in str(flag_sources):
            return True

        # Check match_details affiliation as backup
        match_details = donor.get('match_details', '')
        if match_details:
            try:
                if isinstance(match_details, str):
                    import ast
                    match_dict = ast.literal_eval(match_details)
                else:
                    match_dict = match_details

                # Check both 'affiliation' and 'affiliation_full'
                affiliation = match_dict.get('affiliation', '')
                affiliation_full = match_dict.get('affiliation_full', '')

                if megadonor_name in str(affiliation) or megadonor_name in str(affiliation_full):
                    return True
            except:
                pass
    ...
    nrcc_mask = flagged_donors.apply(lambda row: is_megadonor_type(row, 'NRCC Megadonor'), axis=1)
    republican_mask = flagged_donors.apply(lambda row: is_megadonor_type(row, 'Republican Megadonor'), axis=1)
    rga_mask = flagged_donors.apply(lambda row: is_megadonor_type(row, 'RGA Donor'), axis=1)

    # Extract megadonor DataFrames
    nrcc_donors = flagged_donors[nrcc_mask]
    republican_donors = flagged_donors[republican_mask]
    rga_donors = flagged_donors[rga_mask]

    # Remaining donors (exclude all megadonors)
    megadonor_mask = nrcc_mask | republican_mask | rga_mask
    remaining_donors = flagged_donors[~megadonor_mask]

    return nrcc_donors, republican_donors, rga_donors, remaining_donors

def display_megadonor_section(nrcc_donors, republican_donors, rga_donors):
    """Display megadonor matches grouped by type at the top"""
    # Calculate totals
    nrcc_count = len(nrcc_donors)
    republican_count = len(republican_donors)
    rga_count = len(rga_donors)
    total_megadonors = nrcc_count + republican_count + rga_count

    if total_megadonors == 0:
        return  # Don't show section if no megadonors

    # Display header
    st.markdown("---")
    st.subheader(f"💰 Megadonor Matches ({total_megadonors} total)")
    st.caption("High-value donors with significant Republican giving history")

    # Display each megadonor category in expanders
    if nrcc_count > 0:
        with st.expander(f"🔵 NRCC Megadonors ({nrcc_count})", expanded=False):
            st.caption("Major donors to the National Republican Congressional Committee")
            display_donor_list(nrcc_donors)

    if republican_count > 0:
        with st.expander(f"🟣 Republican Megadonors ({republican_count})", expanded=False):
            st.caption("Significant Republican Party donors")
            display_donor_list(republican_donors)

    if rga_count > 0:
        with st.expander(f"🟢 RGA Donors ({rga_count})", expanded=False):
            st.caption("Donors to the Republican Governors Association")
            display_donor_list(rga_donors)
...
def display_dashboard_cards(flagged_donors):
    ...
    # First, filter out megadonors and display them at the top
    nrcc_donors, republican_donors, rga_donors, remaining_donors = filter_megadonors(flagged_donors)
    display_megadonor_section(nrcc_donors, republican_donors, rga_donors)

    # Use remaining donors (non-megadonors) for the Investigation Dashboard
    if remaining_donors.empty:
        st.info("All flagged donors are megadonors (shown above).")
        return

    # Group remaining donors by priority
    bad_employer = remaining_donors[remaining_donors['flags'].str.contains('BAD_EMPLOYER', na=False)]
    high_conf = remaining_donors[remaining_donors['confidence_level'] == 'HIGH']
    medium_conf = remaining_donors[remaining_donors['confidence_level'] == 'MEDIUM']
    ...
    for _, donor in donors_df.iterrows():
        ...
        with st.expander(title):
            ...
            with col2:
                ...
                # Display full affiliation if available (for all matches)
                if 'match_details' in donor and donor['match_details']:
                    try:
                        if isinstance(donor['match_details'], str):
                            import ast
                            match_dict = ast.literal_eval(donor['match_details'])
                        else:
                            match_dict = donor['match_details']

                        # Show full affiliation if it differs from the shortened version
                        if 'affiliation_full' in match_dict:
                            full_affiliation = match_dict['affiliation_full']
                            shortened_affiliation = match_dict.get('affiliation', match_dict.get('source', ''))
                            # Only show if full text is different from shortened version
                            if full_affiliation != shortened_affiliation:
                                st.write(f"**Full Affiliation:** {full_affiliation}")
                    except:
                        pass

                # Add investigation notes
                if 'BAD_EMPLOYER' in flags:
                    st.error("**Flag:** Bad employer data detected")
                elif confidence == 'HIGH':
...
def display_flagged_donors_by_priority(flagged_donors):
    ...
    sections = [
        ("🏢 Bad Employer Matches", bad_employer, "Flagged employers requiring investigation"),
    ...
    with st.expander(title):
        ...
        with col2:
            ...
            if donor['flag_sources']:
                ...
                # Show full affiliation if available (for all matches)
                if 'match_details' in donor and donor['match_details']:
                    try:
                        # Parse match_details if it's a string
                        if isinstance(donor['match_details'], str):
                            import ast
                            match_dict = ast.literal_eval(donor['match_details'])
                        else:
                        ...
                        pass  # Silently fail if we can't parse match_details
...
def display_industry_detail(industry_df):
    ...
    for _, row in industry_df.iterrows():
        ...
        # DISABLED FOR PERFORMANCE: Get full industry data including org_type
        # if row['committee_id']:
        #     full_data = processor.get_industry_full_data_by_id(row['committee_id'])
        #     if full_data:
        #         enhanced_data.update(full_data)
        
        # DISABLED FOR PERFORMANCE: If no ID data, try fuzzy name matching
...
def display_lpac_detail(lpac_df):
    ...
    for _, pac in grouped_pacs.iterrows():
        # DISABLED FOR PERFORMANCE: Get sponsor district if available
        sponsor_district = "No Sponsor District Available"
        # if pac['committee_id']:
...
def display_results(results):
    ...
    with tab2:
        ...
        if not bad_donors_df.empty:
            ...
            if 'original_df' in st.session_state and 'current_processor' in st.session_state:
                ...
                display_state = geographic_focus_state or filer_state
                
                # Get geographic analysis
                geographic_data = current_processor.get_geographic_analysis(bad_donors_df)
                
                # For state breakdown, use filer_state for FEC reports or geographic_focus_state for State/Local reports
                reference_state = filer_state if filer_state else geographic_focus_state
                state_breakdown = current_processor.get_state_breakdown(bad_donors_df, reference_state)
                        ...
                        if available_states:
                            ...
                            if st.button("Switch to Selected State", type="primary", use_container_width=True):
                                if selected_state_option != current_display:
                                    ...
                                    st.success(f"Switched to {new_state}")
                                    st.rerun()
                                else:
                                    st.info("Already viewing the selected state")
    ...
    with tab3:
        ...
        # Enhanced metrics row for state/local reports and bulk FEC reports
        if 'file_format' in st.session_state and st.session_state.file_format in ["State or Local Report", "Bulk FEC Report"]:
            ...
            if 'date' in bad_donors_df.columns and not bad_donors_df['date'].isna().all() and 'amount_numeric' in bad_donors_df.columns:
                st.markdown("### 📈 Contributions Over Time", help="The totals are of itemized contributions from individuals, totals may varry based on data quality.")
                ...
                if not df_with_dates.empty:
                    # Group by date and sum contribution amounts
                    daily_amounts = df_with_dates.groupby(df_with_dates['date'].dt.date).agg({
                        'amount_numeric': 'sum'  # Sum dollar amounts per day
                    }).reset_index()
                    daily_amounts.columns = ['Date', 'Amount']
                    
                    # Create cumulative amounts over time
                    daily_amounts = daily_amounts.sort_values('Date')
                    daily_amounts['Cumulative_Amount'] = daily_amounts['Amount'].cumsum()
                    
                    # Display both daily and cumulative charts
                    chart_type = st.radio("Chart Type:", ["Daily Amount", "Cumulative Total"], horizontal=True)
                    
                    if chart_type == "Daily Amount":
                        st.line_chart(daily_amounts.set_index('Date')['Amount'])
                        st.caption("📊 Dollar amount of contributions received each day")
                    else:
                        st.line_chart(daily_amounts.set_index('Date')['Cumulative_Amount'])
                        st.caption("📈 Cumulative dollar total of contributions over time")
                else:
        ...
        if ('file_format' in st.session_state and 
            ...
            with col8:
                max_out_donors = derived_metrics.get('max_out_donors', 0)
                st.metric(
                    "Number of Max Out Contributions", 
                    f"{max_out_donors:,}",
                    help="Number of contributions in the $3,400-$3,600 max out range"
                )
            ...
            with col9:
                total_contributions = derived_metrics.get('total_individual_contributions', 0)
                st.metric(
                    "Total Number of Itemized Contributions", 
                    f"{total_contributions:,}",
                    help="Total number of itemized contributions"
                )
            
            with col10:
                median_contribution = derived_metrics.get('median_contribution', 0)
                st.metric(
                    "Median Itemized Contribution", 
                    f"${median_contribution:.2f}",
                    help="Median itemized contribution amount"
                )
            ...
            if 'date' in bad_donors_df.columns and not bad_donors_df['date'].isna().all() and 'amount_numeric' in bad_donors_df.columns:
                ...
                if not df_with_dates.empty:
                    ...
                    # Display both daily and cumulative charts
                    chart_type = st.radio("Chart Type:", ["Daily Amount", "Cumulative Total"], horizontal=True, key="fec_chart_type")
...
def main():
    ...
    if input_method == "Upload File":
        ...
        file_format = st.sidebar.radio(
            "Select file format:",
            ["FEC Quarterly Report", "State or Local Report", "Bulk FEC Report"]
        )
        ...
        elif file_format == "Bulk FEC Report":
            with st.sidebar.expander("📋 About Bulk FEC Reports", expanded=False):
                st.markdown("""
                **When to Use:**
                - After pulling all reciepts from a candidate committee on the FEC Website

                **Step-by-Step:**
                1. **Pull all recipets by clicking "browse reciepts" on the FEC candidate profile page
                2. **Upload your file** using the file uploader below
                4. **Set focus state** (optional) for ZIP code detail maps
                5. **Click Analyze** to process and flag donors

                **Note:** Do not change column positions of the exported file from the FEC if you do, use "State or Local Report" format for manual column mapping.
                """)
        ...
        st.sidebar.info("💡 **Disclaimer:** This tool is used to visualize data but any research or totals used from it should be validated independently.")
        ...
        if uploaded_file:
            ...
            if df is not None:
                ...
                if file_format == "State or Local Report":
                    ...
                    # Get available columns (empty string for easier selection)
                    ...
                    geographic_focus_state = st.text_input(
                        ...
                        value="",
                        max_chars=2,
                        help="Enter a 2-letter state code to focus the ZIP code detail map and enable in-state/out-of-state analysis. Leave blank to determine automatically from data.",
                    ...
                    missing_required = [field for field in required_fields if field == '' or field == '(None)']

                    if missing_required:
                        st.warning(f"⚠️ Please map all required fields. Missing: {', '.join(['First Name', 'Last Name', 'State', 'Amount'][i] for i, field in enumerate(required_fields) if field == '' or field == '(None)')}")
                        ...
                        st.session_state['column_mapping'] = {
                            ...
                            'state': state_col,
                            'amount': amount_col,
                            'zip_code': zip_col if zip_col and zip_col != '(None)' else None,
                            'date': date_col if date_col and date_col != '(None)' else None,
                            'employer': employer_col if employer_col and employer_col != '(None)' else None,
                            'address': address_col if address_col and address_col != '(None)' else None
                        }

                elif file_format == "Bulk FEC Report":
                    # Define the auto-mapping for Bulk FEC Report
                    bulk_mapping = {
                        'first_name': 'S',
                        'last_name': 'U',
                        'state': 'Z',
                    ...
                    # Verify that all required columns exist in the uploaded file
                    missing_columns = []
                    available_columns = list(df.columns)

                    for field_name, col_name in bulk_mapping.items():
                        if col_name and col_name not in available_columns:
                            missing_columns.append(f"{field_name} → Column {col_name}")

                    if missing_columns:
                        st.error(f"⚠️ **Missing required columns:** {', '.join(missing_columns)}")
                        st.warning("Your file is missing the expected columns. Please use 'State or Local Report' format for manual column mapping.")
                        analyze_button_disabled = True
                    else:
                        analyze_button_disabled = False
                        # Store column mapping in session state
                        st.session_state['column_mapping'] = bulk_mapping

                    # Geographic focus state input (same as State/Local)
                    st.markdown("---")
                    st.write("**Geographic Analysis Settings:**")
                    geographic_focus_state = st.text_input(
                        "Focus State for ZIP Detail Map (e.g., NY, CA, TX):",
                        ...
                        max_chars=2,
                        help="Enter a 2-letter state code to focus the ZIP code detail map and enable in-state/out-of-state analysis. Leave blank to determine automatically from data.",
                        key="geographic_focus_state_bulk"
                    ).upper().strip()
                    ...
                    target_donation_amount = st.number_input(
                        ...
                        max_value=10000,
                        help="Donors within +/- $100 of this amount will be counted as 'Max Donors'",
                        key="target_donation_amount_bulk"
                    )
                ...
                if st.button("🔍 Analyze Data", type="primary", disabled=analyze_button_disabled):
                    with st.spinner("Processing donor data and checking against databases..."):
                        try:
                            if file_format == "State or Local Report" or file_format == "Bulk FEC Report":
                ...
                if candidates:
                    ...
                    for i, candidate in enumerate(candidates[:10]):  # Show top 10
                        with st.expander(f"{candidate['name']} ({candidate['party']}) - {candidate['office']} {candidate['state']}"):
                            ...
                            # DISABLED FOR PERFORMANCE: Committee API lookup
                            ...
                            st.info("Committee lookup disabled for performance - see README for restoration instructions")
            
            elif search_type == "Committees":
                # DISABLED FOR PERFORMANCE: Committee search API
                # with st.spinner("Searching committees..."):
                #     committees = api_client.search_committees(search_query)
                st.info("Committee search disabled for performance - see README for restoration instructions")
                committees = []
