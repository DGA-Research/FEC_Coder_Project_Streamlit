    ...
    def _process_all_individual_donors(self, df):
        ...
        for _, row in df.iterrows():
            ...
            donor_data = {
                ...
                'flag_sources': '',
                'confidence_level': '',
                'match_details': '',  # Store full match dict for dropdown display
            ...
            if all_flags:
                ...
                # Store first match details for dropdown display (contains affiliation_full)
                donor_data['match_details'] = str(all_flags[0]) if all_flags else ''
    ...
    def _check_bad_donors(self, donor_data):
        ...
        try:
            ...
            if result:
                ...
                full_affiliation = result[0]
                # Shorten Iowa Lobbyist affiliations for title display
                if full_affiliation.startswith('Iowa Lobbyist'):
                    affiliation_short = 'Iowa Lobbyist'
                else:
                    affiliation_short = full_affiliation[:200]

                flags.append({
                    'flag': 'BAD_DONOR',
                    'source': affiliation_short,  # Use shortened version for title
                    'affiliation_full': full_affiliation,  # Store full text for dropdown
                    'confidence': 'HIGH'
                })
